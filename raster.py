...
def build_mask(shape, v_boxes, r_boxes):
    mask=np.zeros(shape,np.uint8); h,w=shape
...
def process_page(pdf: Path, p: int, root: Path):
    ...
    notes = ([{"id":f"V{i:04d}",**v} for i,v in enumerate(vect)] +
             [{"id":f"R{j:04d}",**r} for j,r in enumerate(rast)])

    page_dir = root / f"page{p:03d}"
    page_dir.mkdir(parents=True, exist_ok=True)
    json.dump(notes,(page_dir/"sidecar_text.json").open("w"),
    ...
    print(f"page {p}: {len(notes):3d} texts masked  "
          f"[{time.time()-t0:.1f}s]")
