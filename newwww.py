...
# -*- coding: utf-8 -*-
"""
pdf_mask_all.py  –  extract *all* text first, then mask the requested pages.
...
# ▸▸  CHANGE THIS to the *bin* folder that contains `pdfinfo.exe`
#     and `pdftoppm.exe` on your machine:
POPPLER_PATH = (
    r"C:\Users\anish.nair\OneDrive - BAE Systems Inc\Desktop\poppler-24.08.0\Library\bin"
)
print("Using poppler from:", POPPLER_PATH)

# ───────────────── 2.  force UTF-8 for the Windows console ─────────
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8", errors="replace")
sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding="utf-8", errors="replace")
os.environ["PYTHONIOENCODING"] = "utf-8"          # makes logging & print() UTF-8
...
def build_mask(shape, v_boxes, r_boxes) -> np.ndarray:
    """Return a solid mask covering every bbox in the two lists."""
    mask = np.zeros(shape, np.uint8)
    h, w = shape
    for x1, y1, x2, y2 in (*v_boxes, *r_boxes):      # loop once over both lists
        cv2.rectangle(
            mask,
...
def dump_json_utf8(path: Path, obj) -> None:
    """Always write binary UTF-8 → Windows code-page is never consulted."""
