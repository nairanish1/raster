#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
pdf_mask_all.py – extract every text string, then blank them on
requested pages.  UTF-8 side-car file → no charmap errors on Windows.
"""

from __future__ import annotations
from pathlib import Path
import json, time, re, io, os, sys

import cv2, numpy as np, typer, easyocr
from pdf2image import convert_from_path, pdfinfo_from_path
from PIL import Image
from pdfminer.high_level import extract_pages
from pdfminer.layout import LTTextContainer

POPPLER_PATH = (
    r"C:\Users\anish.nair\OneDrive - BAE Systems Inc\Desktop"
    r"\poppler-24.08.0\Library\bin"
)
print("Using poppler from:", POPPLER_PATH)

# console UTF-8 (optional, keeps emoji etc. visible)
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8", errors="replace")
sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding="utf-8", errors="replace")

DPI, PAD_VEC, PAD_RAS = 300, 5, 8
reader = easyocr.Reader(["en"], gpu=False, verbose=False)

# ---------- helpers ----------
def pdf_pages(pdf: Path) -> int:
    return pdfinfo_from_path(str(pdf), poppler_path=POPPLER_PATH)["Pages"]

def parse_pages(spec: str, total: int) -> list[int]:
    if spec.lower() == "all":  return list(range(total))
    if re.fullmatch(r"\d+(,\d+)+", spec):
        return [int(x) for x in spec.split(",")]
    m = re.fullmatch(r"(\d+)\.\.(\d+)", spec)
    return list(range(int(m[1]), int(m[2])+1)) if m else [int(spec)]

def vector_boxes(pdf: Path, idx: int):
    out = []
    for p, layout in enumerate(extract_pages(str(pdf))):
        if p != idx:  continue
        for obj in layout:
            if isinstance(obj, LTTextContainer):
                x1,y1,x2,y2 = map(int, obj.bbox)
                txt = obj.get_text().strip()
                if txt: out.append({"bbox":[x1,y1,x2,y2], "text":txt})
    return out

def raster_page(pdf: Path, idx: int):
    img = convert_from_path(str(pdf), dpi=DPI,
                            first_page=idx+1, last_page=idx+1,
                            fmt="png", poppler_path=POPPLER_PATH)[0].convert("L")
    return np.array(img)

def raster_boxes(gray: np.ndarray):
    h,w = gray.shape; out=[]
    for pts, txt, _ in reader.readtext(gray, detail=1, paragraph=False):
        xs=[int(p[0]) for p in pts]; ys=[int(p[1]) for p in pts]
        x1=max(0,min(xs)-PAD_RAS); y1=max(0,min(ys)-PAD_RAS)
        x2=min(w,max(xs)+PAD_RAS); y2=min(h,max(ys)+PAD_RAS)
        out.append({"bbox":[x1,y1,x2,y2], "text":txt.strip()})
    return out

def build_mask(shape, boxes):
    mask = np.zeros(shape, np.uint8)
    h,w = shape
    for x1,y1,x2,y2 in boxes:
        cv2.rectangle(mask, (max(0,x1-PAD_VEC), max(0,y1-PAD_VEC)),
                             (min(w,x2+PAD_VEC), min(h,y2+PAD_VEC)), 255, -1)
    return cv2.morphologyEx(mask, cv2.MORPH_CLOSE, np.ones((3,3),np.uint8))

# ---------- per page ----------
def process_page(pdf:Path, idx:int, root:Path):
    t0=time.time()
    gray=raster_page(pdf,idx)
    v=vector_boxes(pdf,idx); r=raster_boxes(gray)
    notes=[*({"id":f"V{i:04d}",**x} for i,x in enumerate(v)),
           *({"id":f"R{j:04d}",**x} for j,x in enumerate(r))]

    pg=root/f"page{idx:03d}"; pg.mkdir(parents=True,exist_ok=True)

    # *** THE ONLY LINE THAT MATTERS ↓↓↓ ***
    with (pg/"sidecar_text.json").open("w", encoding="utf-8") as f:
        json.dump(notes,f,indent=2,ensure_ascii=False)

    geom=gray.copy(); geom[ build_mask(gray.shape,[b["bbox"] for b in v+r])>0]=255
    Image.fromarray(gray).save(pg/"page_raw.png")
    Image.fromarray(geom).save(pg/"geom_only.png")
    print(f"page {idx:03d}: {len(notes):4d} texts masked [{time.time()-t0:.1f}s]")

# ---------- CLI ----------
cli = typer.Typer()

@cli.command(no_args_is_help=True)
def main(pdf:str=typer.Option(...),
         pages:str=typer.Option("all"),
         out:str=typer.Option("build/coordination")):
    p=Path(pdf).expanduser().resolve()
    ids=parse_pages(pages,pdf_pages(p))
    print(f"Processing pages {ids} of {p.name}")
    root=Path(out).expanduser()
    for i in ids: process_page(p,i,root)

if __name__ == "__main__":
    cli()



