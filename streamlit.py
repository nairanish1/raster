...
def load_and_clean(excel_bytes: bytes):
    ...
    df['Total FAN Reviews'] += df['RM'].apply(
        lambda x: 1 if pd.notna(x) and x != 0 else 0
    )
    df['Total FAN Reviews'] += df['CI'].apply(
        lambda x: max(0, x - 1) if pd.notna(x) else 0
    )

    # merge critical flag
    df = df.merge(
        df_critical[['PNOC ID', 'critical']], on='PNOC ID', how='left'
    )
...
def analyze(df: pd.DataFrame, types: list[str], min_fans: int):
    ...
    df = df.dropna(subset=['Baseline', 'Actual'])
    if df.empty:
        return None, None
    ...
    # average per PNOC
    bsa = df.groupby('PNOC ID')['Business_Days_Difference']\
            .mean()\
...
def main():
    ...
    types = st.sidebar.multiselect(
        "Choose PNOC categories", ["Routine","Critical"],
        default=["Routine","Critical"]
    )
    ...
    if st.sidebar.button("Run Analysis"):
        ...
        if bsa is None:
            st.warning(
                "No data after filtering — try lowering the FAN reviews or include both types."
            )
            return
